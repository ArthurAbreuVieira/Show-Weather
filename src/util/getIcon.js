...
export default function getIcon(id, date, size, color) {
  if(String(id).startsWith('2')) {
    return <Ionicons name="thunderstorm-outline" size={size} color={color||"#4ac0ff"} />;
  } else if(String(id).startsWith('3')) {
    return <Feather name="cloud-drizzle" size={size} color={color||"#4ac0ff"} />;
  } else if(String(id).startsWith('5')) {
    return <Fontisto name="rain" size={size} color={color||"#4ac0ff"} />
  } else if(String(id).startsWith('6')) {
    return <Fontisto name="snowflake-8" size={size} color={color||"#4ac0ff"} />
    ...
    if(Number.parseInt(convertTimestamp(date).hour) >= 5 && Number.parseInt(convertTimestamp(date).hour) <= 17) {
      ...
      return <Ionicons name="md-moon" size={size} color={color||"#0051ba"} />
    }
  } else if(id == '801') {
    if(Number.parseInt(convertTimestamp(date).hour) >= 5 && Number.parseInt(convertTimestamp(date).hour) <= 17) {
    ...
    return <Ionicons name="ios-cloudy" size={size} color={color||"#666"} />
  }
}